<template>
    <div>
      <div class="relative grid grid-cols-1 gap-x-14 _md:grid-cols-2">
        <button @click="lastMonth()" type="button" class="absolute -left-1.5 -top-1 flex items-center justify-center p-1.5 text-gray-400 hover:text-gray-500">
          <span class="sr-only">Previous month</span>
          <ChevronLeftIcon class="h-5 w-5" aria-hidden="true" />
        </button>

        <button @click="nextMonth()" type="button" class="absolute -right-1.5 -top-1 flex items-center justify-center p-1.5 text-gray-400 hover:text-gray-500">
        ...
        <section class="text-center">
          <h2 class="text-sm font-semibold text-gray-900">{{ month }} {{ year }}</h2>
          <div class="mt-6 grid grid-cols-7 text-xs leading-6 text-gray-500">
            <div>M</div>
            <div>T</div>
            <div>W</div>
          ...
          <div class="isolate mt-2 grid grid-cols-7 gap-px rounded-lg bg-gray-200 text-sm shadow ring-1 ring-gray-200">
            <button 
                ...
                :key="index" 
                :class="[
                    day == initialDate ? 'bg-white text-gray-900' : 'bg-gray-50 text-gray-400', 
                    index === 0 && 'rounded-tl-lg', 
                    index === 6 && 'rounded-tr-lg', 
                    // index === month.days.length - 7 && 'rounded-bl-lg', 
                    // index === month.days.length - 1 && 'rounded-br-lg',
                ]"
            ...
            >
              <!-- <time 
                :datetime="day.date" 
                :class="[day.isToday && 'bg-indigo-600 font-semibold text-white']"
...
const year = computed(() => dateContext.value.format('YYYY'))

const month = computed(() => dateContext.value.format('MMMM'))

const daysInMonth = computed(() => dateContext.value.daysInMonth())

// const currentDate = computed(() => dateContext.value.date())
const currentDate = computed(() => dateContext.value.get('date'))

const firstDayofMonth = computed(() => {
    const firstDay = moment(dateContext.value).subtract((currentDate - 1), 'days')
    return firstDay.weekday()
})

const initialDate = computed(() => today.value.get('date'))

const initialMonth = computed(() => today.value.format('MMMM'))

const initialYear = computed(() => today.value.format('YYYY'))


function nextMonth() {
    dateContext.value = moment(dateContext.vlaue).add(1, 'month');
}

function lastMonth() {
    dateContext.value = moment(dateContext.value).subtract(1, 'month');
}
